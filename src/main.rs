...
use xcb::{MAP_REQUEST, CW_EVENT_MASK, EVENT_MASK_SUBSTRUCTURE_REDIRECT, EVENT_MASK_SUBSTRUCTURE_NOTIFY, Window, GenericEvent, CONFIG_WINDOW_WIDTH};
use xcb;
use std::process::Command;
fn main() {
    let (connection, _) = Connection::connect(Some(":0")).unwrap();
    ...
    while let Some(event) = &connection.wait_for_event() {
        match event.response_type() {
            MAP_REQUEST => {
                ...
                //xcb::set_input_focus(&connection, XCB_INPUT_FOCUS_POINTER_ROOT as u8, casted_event.window(), 0);
               // xcb::configure_window(&connection, casted_event.window(), &values);
                manage_windows(&connection, (screen.width_in_pixels() as u32, screen.height_in_pixels() as u32), &windows);
...
fn set_wallpaper() {
    Command::new("sh")
        .args(["-c", "DISPLAY=:0 hsetroot -cover /home/satbh/wallpapers/everforest"])
        .output()
        .unwrap();
}
